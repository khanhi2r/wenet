    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            if len(value) == 3:
                ...
                if len(waveform[0]) < 400:
                    raise Exception(f"sox {waveform.shape} {wav_path} {start_frame} {end_frame}")
            else:
...
class AudioDataset(Dataset):
    def __init__(self, data_file: str, sample: float=1.0):
        self.items = []
        if sample <= 0:
            raise RuntimeError("sample must be greater than 0")
            ...
            with codecs.open(data_file, 'r', encoding='utf-8') as f:
                for line in f:
                    if random.random() < sample:
                        arr = line.strip().split()
                        self.items.append((arr[0], arr[1]))
        print(f"cmvn_loaded: {len(self.items)} items")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample',
                        default=1.0,
                        type=float,
                        help='sample an small amount of data to calculate cmvn')
    ...
    dataset = AudioDataset(args.in_scp, args.sample)
    batch_size = 20
