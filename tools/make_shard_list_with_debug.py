...
def write_tar_file(data_list,
    ...
    with tarfile.open(tar_file, "w") as tar:
        ...
        for item in data_list:
            ...
            suffix = wav.split('.')[-1]
            assert suffix.lower() in AUDIO_FORMAT_SETS, f"suffix {suffix} is not in audio format sets {AUDIO_FORMAT_SETS}"
            if no_segments:
                # read & resample
                ts = time.time()
        ...
        for line in fin:
            ...
            if no_segments:
                ...
                if key not in segments_table:
                    print(f"WARNING key not in segment table: {key}", file=sys.stderr)
                    continue
